...
public class PacketCount {
	...
	static final String OUTPATH = "PcapPacketCount_out";	
	...
	public static void main(String[] args) throws Exception{
		...
		while(i<args.length){
			if(args[i].startsWith("-")){
				...
				// map and reduce artificial delay (ms)	
				case 'X': case 'x':
					MAPms = Long.parseLong(args[i]);
					break;			
				case 'Y': case 'y':
					REDms = Long.parseLong(args[i]);
					break;			
...
//			Path inputPath = new Path(srcFilename);
			...
			hcoral.startPacketCount(inputPath, outputDir, cap_start, cap_end, MAPms, REDms);
		}
	}
}
